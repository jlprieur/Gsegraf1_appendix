...
// Get current plot values 
   ...
   jlp_gsegraf0->GSEG_GetBoxSettingsForLinear(&dev_x1_box, &dev_x2_box, 
                                              &dev_y1_box, &dev_y2_box, 
                                              &xmin, &xmax, &ymin, &ymax, 
                                              &zmin, &zmax, &xscale, &yscale, 
                                              axis_type);
      ...
      case GDK_BUTTON_PRESS:
         if ( flag_2d_rect == 1 )
            ...
            if ( ((GdkEventButton*) event)->button == 1 &&
               ...
               if ( x1_window < dev_x1_box )
                  x1_window = dev_x1_box;
               else if ( x1_window > dev_x2_box )
                  x1_window = dev_x2_box;

               if ( y1_window < dev_y1_box )
                  y1_window = dev_y1_box;
               else if ( y1_window > dev_y2_box )
                  y1_window = dev_y2_box;
...
/* Grab mouse pointer */
            ...
            if ( x2_window < dev_x1_box )
               x2_window = dev_x1_box;
            else if ( x2_window > dev_x2_box )
               x2_window = dev_x2_box;

            if ( y2_window < dev_y1_box )
               y2_window = dev_y1_box;
            else if ( y2_window > dev_y2_box )
               y2_window = dev_y2_box;
...
/* Draw rectangle */
            g_object_set(zoom_rectangle, "x1", x1, "x2", x2, "y1", y1, 
                         "y2", y2, NULL);
            }
...
/* Write x and y values to plot window (bad value is -1234.) */
            memset(string, 0, sizeof(string));
            if ( xy_coords_data[0] != -1234. && xy_coords_data[1] != -1234. )
               {
               if ( flag_2d_rect == 1 )
...
/* Write z value below cursor position (bad value is -1234.) */
            memset(string, 0, sizeof(string));
            if ( z_coords_data != -1234. )
               {
               if ( flag_2d_rect == 1 )
...
/* Re-initialize plot canvas */
               jlp_gseg0->GSEG_gnome_InitializePlotCanvas(group, 
                                                          height_menu_bar);

/* Redraw plot */
               jlp_gsegraf0->GSEG_ZoomIn(x1_window, y1_window, x2_window, 
                                         y2_window);
               }
...
// Calculate dash parameters:
         ...
         jlp_gsegraf0->GSEG_SetDashParameters(tick_major_0, tick_minor_0,    
