...
// Create the string that will be displayed for prompting new axes limits:
   GSEG_QuestionForChangingAxesLimits(prompt, string);
...
// Decode the input string and change the axes limits
 status = GSEG_ChangeAxesLimits(string, &xmin, &xmax, &ymin, &ymax,
...
/* Redraw plot */
 GSEG_ReDrawPlot_Scaled(xmin, xmax, ymin, ymax, zmin, zmax,
                        &dcflag);
...
char string[128], prompt[512];
   ...
   /* Create dialog box */
  status = GSEG_QuestionForChangingAxesRotation(prompt, string);
   if ( status == 0 )
      {
/* Open dialog box */
...
/* Draw plot */
         GSEG_ReDrawPlot3D_Rotated(phi, theta);
         }
      else
         {
...
int ncontour_plots0; 
   ...
   GSEG_get_ncontour_plots(&ncontour_plots0);
...
/* Check icontour_plots value */
   ...
   GSEG_CreateContourLabel(xmouse, ymouse, icontour_plots, &zcontour_data,
                           &contour_label);
...
// Get current plot types
   GSEG_SetPointersFromPlot(&plot_types_0);
...
/* Get number of color or contour plots */
      ...
      for ( iplot=1; iplot<=nplots; iplot++ )
         {
         if ( strcmp(&plot_types_0[(iplot-1)*10], "color") == 0 )
            {
            icount_color++;
            plot_type_data[iplot-1] = icount_color;
            }
         else if ( strcmp(&plot_types_0[(iplot-1)*10], "contour") == 0 )
            {
            icount_contour++;
...
// Contour plots
 ...
 GSEG_UserFromDeviceCoord(xmouse, ymouse, plot_type_data0,
                          &xy_coords_data[0],
